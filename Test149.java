...
==========================================================*/
// 프레임워크 "약속"
// 프레임워크라고 하기엔 사이즈가 민망ㅠ 보통은 컬렉션이라고 부른다
// 컬렉션은 자료구조 어떻게 쓰느냐는 알고리즘
// 무엇을 효율적으로 어떻게(어디에) 담느냐
// List - 순서가 있는 자료구조 / 중복 저장 가능
// Set  - 순서x 뒤섞여있는 자료구조 / 중복 저장 불가
// Map  - 뒤섞여있지만 바깥에 순서의 번호가 이어져있는 자료구조 (키와 값 / 키는 중복 저장 불가)
...
==> 자료구조
 ...
 ○ 자료구조
	...
	처리를 위한 알고리즘의 작성 및 선택과 이름 방법을 연구하는 분야
 ...
 ○ 컬렉션 프레임워크(Collection Framework)
	...
	1. 인터페이스(Interface)
		...
		예를들어, list는 추가, 삭제, 검색등의 기능을 제공하는
		컬렉션 인터페이스가 존재하며
		각 컬렉션마다 공유의 인터페이스가 정의되어 있다.
...
/*
 ○ 스택(Stack)
	...
	Stack 클래스는 오브젝트 후입선출(LIFO : Last Input First Out) 구조의 Stack을 나타낸다
	이는 벡터(Vector)를 스택으로 처리하는 5개의 메소드로
	벡터(Vector) 클래스를 확장하게 된다

	통상으로 push() 메소드와 pop()메소드가 제공되는 것 외에
	스택의 선두 항목으로 peek()을 실시하는 메소드로
	스택이 비어있는지의 여부를 확인하는 메소드 isEmpty()
	스택으로 항목을 찾아서 선두로부터 몇 번째인지를 찾아네는 메소드 등이 제공된다.
...
public class Test149
{
	public static void main(String[] args)
	{
		...
		// peek() : 스텍 맨 위의 객체 반환. 제거하지 않는다.
		String val = (String)myStack.peek();	// (String으로 변환)Object타입의 윤비
