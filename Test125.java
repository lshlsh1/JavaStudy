...
/*
 ○ Wrapper 클래스
 	...
 	1. 자바에서는 언어 수준에서 제공하는 기본형 데이터를 제외한
	   ...
	   이러한 클래스를 가리켜 Wrapper 클래스라고 한다.

	2. 기본 자료형인
	   byte, short, int,   , long, float, double, char     , boolean 형에 대응하는
	   Byte, Short, Integer, Long, Float, Double, Character, Boolean 의				-클래스에 들어있음
	   Wrapper 클래스 및
	   큰 숫자들을 다루기 위한
...
○ 오토 박싱(Auto-Boxing) 과 오토 언박싱(Auto-Unboxing)
	...
	Integer ob;
	ob=a;
	// 기본 자료형(int)와 Integer 형으로 자동 변환(오토 박싱)
	// 실제로는.... 『ob = new Integer(a);』 이런 과정을 거침

	b = ob;
	//Object(Integer)가 기본 자료형(int)으로 자동 변환(오토 언박싱)
	// 실제로는 『b = ob.intValue();』

	이는 JDK 1.5 이후 추가된 오토 박싱과 오토 언박싱이라는 기능을 인해 가능해진 것이다.
...
public class Test125
{
	public static void main(String[] args)
	{
		...
		c = new Integer(0); //Scanner와 비슷한 구조 Scanner sc = new Scanner();

		// ※ 레퍼런스 자료형(참조 타입)과 기본 자료형(기본 타입)은 호환되지 않는다.
		//	  JDK 1.5 부터 레퍼런스 형과 기본 자료형이 
		//	  자동으로 형 변환이 가능하게 되었다.

		b = c; //-- Integer형 데이터(객체) 를 → int형 변수 (오토 언박싱)
		// ||
		b = c.intValue();	// (언박싱)
