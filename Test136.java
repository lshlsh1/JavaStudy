...
○ 캐릭터 셋(Character Set)
	...
	- 유니코드(unicode)
	  ...
	  애플컴퓨터, IBM, 마이크로소프트 들이 컨소시엄으로 설립한
	  유니코드(Unicode)가 1990년에 첫 버전을 발표하였고
	  ISO/IEC JTC1 에서 1995년 9월 국제 표준으로 제정하였다.
	  공식 명칭은 ISO/IEC 10646-1(Universe Multiple-Octet Coded
	  Character Set)이다
	...
	- UTF-8 과 UTF-16
	  ...
	  UTF-8은 8비트의 가변창 멀티바이트에서 문자를 표현하고
	  UTF-16은 16비트의 가변창 멀티바이트에서 문자를 표현한다
...
○ 문자코드의 종류
	...
	- EUC-KR(M5949)
	  ...
	  (EUC-KR(M5949) = KSC601 + KSC5636)

	- UTF-8
	  영문과 숫자를 1바이트, 한글은 3바이트로 표현
	  (~웹에서 많이 사용~, UTF-8형식으로 JSP를 작성할 때에는
	   파일도 UTF-8형식으로 저장해야 한다.)
...
public class Test136
{
	public static void main(String[] args)	//throws UnsupportedEncodingException
	{
		...
		try
		{
			...
			// UTF-8로 쪼갠 글자를 기본캐릭터셋(EUC-KR)으로 합침
			//--==>>value2 : ?궓?긽?쁽
			...
			System.out.println("buf 배열의 Lenghth : "+buf.length);
			//--==>>buf 배열의 Lenghth : 6
			...
			/* EUC-KR 을 UTF-8로 변환한다고 위와 같이 코딩을 해버리면
