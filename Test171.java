...
/*
 ...
 ○ Serialization 인터페이스
	- 객체 직렬화를 하기 위해 먼저 객체 직렬화가 가능하도록
	  java.io,Serailizatable 인터페이스를 구현해야 한다
	  이 인터페이스는 객체 직렬화가 제공되어야 한다는 사실을
	  JVM(자바 가상 머신)에게 알려주는 역할을 수행한다
	  또한 Serialization 인터페이스는 다른 인터페이스와는 달리
	  구현해야 할 메소드가 없기 때문에 단지 선언만 해주면 된다

	  형식)
	  ①
	  public class 클래스명 implements Serializatable
	  {
		  	//....
	  }
 ...
 ○ Object Stream
	...
	- java.io.ObjectInputStream
	  ...
	  파일이 저장되어 있는 객체나 네트워크를 통해 전달된 객체의
	  직렬화를 해제하는 기능을 제공한다
	  단, java.io.Serializable 인터페이스와
	  java.io.Externalizable 인터페이스를 지원해주는 객체에 대해서만
	  사용이 가능하다
	  즉, Serializable 인터페이스와 Externavlizable 인터페이스를
