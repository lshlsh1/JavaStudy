...
/*
 ※ 『super』
	...
	하위 클래스의 생성자에서 상위 클래스의 생성자를 호출할 떄에는
	하위 클래스의 생성자 정의 구문에서 맨 처음에만 위치할 수 있따

 ※ 생성자와 클래스 상속 간의 관계

 	하위 클래스는 상위 클래스의 멤버(변수)를 상속받지만
	생성자는 상속 대상에서 제외된다.
	그리고 하위클래스의 생성자가 호출될 때
	자동으로 상위 클래스의 생성자가 호출된다
	이 때 상위 클래스의 생성자는 인수가 없는 생성자(default 생성자)가 호출된다.
...
============================================================================================
	인수가			|	생성자 정의 안함	 | → 가능하다.
	없는			|	인수가 없는 생성자	 | → 가능하다.
	생성자만 정의	|	인수가 있는 생성자	 | → 가능하다. (자동으로 인수가 없는 상위 클래스 생성자구문 삽입)
============================================================================================
	인수가			|	생성자 정의 안함	 | → 에러 발생~!!
...
// p.87
 ...
 ※ 상속 시 주의할 사항
	...
	→ Method Overriding과 구분하여 개념을 정리해 둘 것~!!!
...
class SuperTest02
{
		...
		private String title;	//private 은 상속 대상에서 제외~!!!!
...
class Rect	extends SuperTest02
{
	...
	*/
	...
	//public void write(int a)  // 메소드오버로딩

	@Override					//-- 어노테이션(Annotation) 어떠한 기능은 없지만 부연, 추가, 도와주는것	// jdk1.5이상
	public void write()			// 메소드오버라이딩(메소드 재정의) //가능--상속받았을때는 덮어쓰는 기능 // 물려받은 차 튜닝
	...
	//메소드오버라이딩, 메소드오버로딩과 비교하기
	//인스턴스생성으로 메모리할당받는 것은 변수들 뿐 그 때에 메소드들은 점검만 받는다. 
	//상위 클래스와 하위클래스에서 사용하는 변수의 이름이 같더라도 각각다르게 존재할 수 있다.
...
public class Test107
{
	public static void main(String[] args)
	{
		...
		//--==>> 에러발생 (컴파일 에러) → 사용자 정의 생성자로 인해 default 생성자가 삽입되지 않음
