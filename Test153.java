...
/*
 ...
 ○ Vector(벡터)
	...
	데이터 요소로 가질 수 있으며, 데이터가 가득 차면 자동으로 저장 영역을
	늘려주게 된다.

	또한, 객체 레퍼런스를 저장하기 때문에
	JDK 1.5 이전에는 자바 기본형 데이터는 자정할 수 없었으나
	...
	- void insertElemenAt(E obj, int index)
	  주어진 위치에 객체를 저장한다(밀어넣는 개념)
	  주어진 위치의 객체부터 오른쪽으로 하나씩 이동한다
		1	2	10	3	4	5

	- void addElement(E obj)
	- boolean add(E e)
	  객체를 벡터 끝에 추가한다.(오른쪽 끝에)

	- void removeElemnetAt(int index)
	- E remove(int index)
	  주어진 위치의 객체를 제거한다
	...
	- E set(int index, E element)
	  ...
	  주어진 위치에 저장되어있던 객체는 소멸한다 → 덮어쓰기 개념
	...
	- Enumeration<E> elements()
	  벡터의 모든 요소를 반환한다
	  Enumeration<E> 전체가 리턴 자료
	  비슷한 것은 Iterator 컬렉션을 도와줌(나중에 다시 나옴)
...
public class Test153
{
	...
	private static final String[] colors = {"검정", "노랑", "초록", "파랑", "빨강", "연두"}; 
		...
		public static void main(String[] args)
		{
			...
			//v라는 벡터 자료구조에 color 배열에 담겨있는 데이터 담아내기
			//add()
			/*
